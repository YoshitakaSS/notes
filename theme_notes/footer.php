<footer id="footer" itemscope="itemscope" itemtype="http://schema.org/WPFooter">
        <div class="bottom-list container">
            <div class="profile list-wrap">
                <hr>
                <h4>Profile</h4>
                <hr>
                <div class="profile-info">
                    <p class="prof-nm" itemprop="author">
                        Anonymous
                    </p>
                    <p>Enter Profile
                        dummy Textdummy Textdummy Text
                        dummy Textdummy Textdummy Text
                        dummy Textdummy Textdummy Text
                        dummy Textdummy Textdummy Text
                    </p>
                    <!-- 自分のTwitterを埋め込んでください -->
                    <a href="https://twitter.com/sy_sh7711?ref_src=twsrc%5Etfw" class="twitter-follow-button" data-show-count="false">Follow @sy_sh7711</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
                </div>
            </div>
            <div class="related-links list-wrap">
                <hr>
                <h4>Related Links</h4>
                <hr>
                <ul class="links-info">
                    <li><a href="#">Google</a></li>
                    <li><a href="#">Yahoo</a></li>
                    <li><a href="#">Bing</a></li>
                    <li><a href="#">Note</a></li>
                    <li><a href="#">Github</a></li>
                </ul>
            </div>
            <div class="embedded-twiiter list-wrap">
                <!-- https://publish.twitter.com/# で自分のタイムラインを埋め込む -->
                <hr>
                <h4>Twitter</h4>
                <hr>
                <div class="twitter-wrap">
                    <a class="twitter-timeline" data-height="400" data-theme="light" href="https://twitter.com/sy_sh7711?ref_src=twsrc%5Etfw">Tweets by sy_sh7711</a> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
                </div>
            </div>

        </div>
        <div class="footer-copyright">
            <p>&copy; - <span itemprop="copyrightHolder">ys</span> - <span itemprop="copyrightYear">2019</span> All Rights Reserved</p>
        </div>
    </footer>
    <?php wp_footer(); ?>
</body>
    <!-- js -->
    <script src="<?php echo get_template_directory_uri(); ?>/script/jquery-3.1.1.min.js" type="text/javascript"></script>
    <script src="<?php echo get_template_directory_uri(); ?>/script/main.js" type="text/javascript" async></script>    
</html>